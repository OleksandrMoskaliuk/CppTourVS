...
namespace pr_cmm {
	...
	template <class Data>
	static void ProcessCommunicator::send_message(Data dt) {
	...
	template <class Data>
	static void ProcessCommunicator::get_message(Data& dt) {
...
#if ProcessCommunicatorDebug
			std::cout << "Test successful!:" << dt << std::endl;
#endif // ProcessCommunicatorDebug
	...
	void sprocess_1(bool& exit)
	{
		...
		while (!exit)
		{
			...
			// } process stream info 
			ProcessCommunicator::send_message<std::string>("White Mamba");
	...
	void sprocess_2(bool& exit)
	{
		...
		while (!exit)
		{
			...
			// } process stream info 
			ProcessCommunicator::get_message<std::string>(res);
